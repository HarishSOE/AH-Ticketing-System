...
import 'package:ah_ticketing_prod/Widgets/video_thumbnail.dart';
            ...
            uploadTask.whenComplete(() async{
              ...
              if(contentType?.split("/")[0] == "video"){
                try {
                  ...
                  String xFilePath = thumbnailFile.path;
                  if(xFilePath.isNotEmpty){
                    print(thumbnailFile);
                    String thumbnailName = xFilePath.split('/').last;
                    Reference thumbnailReference = FirebaseStorage.instance.ref().child('$table/media/${DateTime.now().toString()}_$thumbnailName}');
                    UploadTask uploadThumbnailTask = thumbnailReference.putFile(File(xFilePath), SettableMetadata(contentType: "image/jpeg"),);
