...
import 'package:ah_ticketing_prod/Widgets/chatBox.dart';
...
class _ChatDetailScreenState extends State<ChatDetailScreen> {
  ...
  StreamSubscription? unreadMsgSubscription;

  Map mapWatiTemplate = {};
  Map mapProfileUid = {};
  Map<String, Map> messageReplayMap = {};
  ...
  void initState() {
    ...
    if(widget.selectWati == "All Chats"){
      filterwatiDocs = ['Event Wati','CS Wati', 'BIG Wati', 'Finance Wati'];
    }else{
      filterwatiDocs = [widget.selectWati];
    }
    _loadProfiles();
    ...
    unreadMsgSubscription = firestore.collection("wati logs").where('conversationId', isEqualTo: widget.latestMessage['conversationId']).snapshots().listen((unreaddoc) async {
      if (unreaddoc.docs.isNotEmpty) {
        WriteBatch batch = firestore.batch();
        String yourId = UserData().auth.currentUser!.uid; 
        for (var unread in unreaddoc.docs) {
          List readBy = unread.data()['readby'] ?? [];

          if (!readBy.contains(yourId)) {
            DocumentReference docRef = unread.reference;
            batch.update(docRef, {
              'readby': FieldValue.arrayUnion([yourId])
            });
          }
        }
        await batch.commit();
        unreadMsgSubscription?.cancel();
      }
    });
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        for (var doc in snapshot.docs) {
          ...
          if(profile['user_ref'] != null && profile['user_ref'] != ""){
            profilesMap[profile['user_ref'].id] = profile;
          }
        }
        
        if (mounted) {
          setState(() {
            mapProfileUid = profilesMap;
          });
        }
      }
    });
  ...
  fetchInitialMessages() {
    ...
    try {
      Query query = firestore.collection("wati logs").where("waId", isEqualTo: widget.waId).where("watiName",whereIn: filterwatiDocs).orderBy("date", descending: true).limit(pageSize);
        ...
        (QuerySnapshot snapshot) {
          ...
          if (snapshot.docs.isNotEmpty) {
            ...
            for (var doc in snapshot.docs) {
              ...
              if(data['replyContextId'] != null && data['replyContextId'] != '' && data['type'] == 'template' && data['type'] != 'delivered'){
                messageReplayMap[whatsappMessageId] = data;
              }
  ...
  Future<void> _loadMoreMessages() async {
    ...
    try {
      QuerySnapshot snapshot = await firestore.collection("wati logs").where("waId", isEqualTo: widget.waId).where("watiName",whereIn: filterwatiDocs).orderBy("date", descending: true).startAfterDocument(lastDocument!).limit(pageSize).get();
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        for (var doc in snapshot.docs) {
          ...
          if(data['replyContextId'] != null && data['replyContextId'] != '' && data['type'] == 'template'){
            messageReplayMap[whatsappMessageId] = data;
          }
  ...
  Widget build(BuildContext context) {
    bool expired = displayedMessages.length > 0 && displayedMessages[displayedMessages.length - 1]['eventType'] == 'message' && isOlderThan24Hours(displayedMessages[displayedMessages.length - 1]['date']) ? true : false;

    return Scaffold(
      backgroundColor: Color.fromARGB(255, 228, 255, 235),
      appBar: AppBar(
        ...
        titleSpacing: 0,
        title: isSearching ? _buildSearchField() : Row(
          children: [
            // Contact avatar
            CircleAvatar(
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: expired ? const Color.fromARGB(255, 245, 105, 95) : Color.fromARGB(255, 210, 255, 233),
          ...
          Container(
            decoration: BoxDecoration(
              image: DecorationImage(
                image: AssetImage('assets/images/chat_background2.jpg'),
              ...
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.search, size: 18, color: AppColors.purple,),
                      SizedBox(width: 6),
                      Text(
                ...
                Expanded(
                  ...
                  ) : displayedMessages.isEmpty ? Center(
                    child: Text(
                      isSearching ? 'No messages match your search' : 'No messages found',
                    )
                  ) : Stack(
                    children: [
                      ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          if(message['replyText'] != '' && message['replyText'] != null){
                            print('${message['replyContextId']}');
                            message['replyText'] = messageReplayMap[message['replyContextId']];
                          }
                          ...
                          return Column(
                            children: [
                              ...
                              if (displayedMessages[displayedMessages.length - 1]['eventType'] == 'message' && isOlderThan24Hours(displayedMessages[displayedMessages.length - 1]['date']) && index == 0)
                              _buildChatExpiryNotice(message),
                            ],
                          );
                        },
                      ),
                      ...
                      Expanded(
                        child: Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                replyingTo!['owner'] == true ? 'You' : widget.contactName,
                                style: GoogleFonts.poppins(
                                  color: AppColors.purple,
                                  fontWeight: FontWeight.bold,
                                  fontSize: 14,
                    ...
                    children: [
                      Expanded(
                        child: TextField(
                          ...
                          enabled: replyingTo != null, // Enable only when replying
                          decoration: InputDecoration(
                            hintText: replyingTo != null ? 'Type a reply...' : 'Select Chat to Tag',
  ...
  updateDocument(messageDoc){

    firestore.collection("wati logs").doc(messageDoc['docid']).update({
      "teamreplay": FieldValue.arrayUnion([
        {
        "taggedby" : UserData().auth.currentUser?.uid ?? null,
        "taggedtime" : DateTime.now(),
        "taggedmsg" : _messageController.text,
        }
      ])
    }).then((value) {
      print('Successfully Tagged Team');
    }).catchError((error){
      print('Oops Error while Tagging Team${error}');
    });
  }
  ...
  Widget _buildSwipeableMessage({
    ...
    return Dismissible(
      ...
      confirmDismiss: (direction) async {
        // Set the message as the one we're replying to
        setReplyingTo(message);
        // Don't actually remove the item
        return false;
      },
      child: chatBox(
        isOwner: isOwner,
  ...
  Widget _buildChatExpiryNotice(Map<String, dynamic> message) {
    return Container(
      ...
      child: Text(
        'The chat has been expired\n(after 24 hours of last received message)',
        style: GoogleFonts.poppins(
          color: Color.fromARGB(255, 248, 77, 77),
          fontSize: 13,
        ),
...
//                 mainAxisAlignment: MainAxisAlignment.center,
//                 children: [
//                   Icon(Icons.access_time, size: 18, color: AppColors.purple,),
//                   SizedBox(width: 6),
//                   Text(
//                     'Chat expires in $expiryTimeDisplay',
//                     style: GoogleFonts.poppins(
...
//     } else {
//       // Match the WhatsApp style: custom AppColors.purple for sent messages, white for received
...
//                                 decoration: BoxDecoration(
//                                   color: Colors.AppColors.purple[700],
//                                   borderRadius: BorderRadius.circular(10),
//                                 ),
//                                 child: Text(
