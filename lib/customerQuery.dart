...
import 'package:ah_ticketing_prod/Themes.dart';
import 'package:ah_ticketing_prod/TicketChat.dart';
import 'package:ah_ticketing_prod/raiseTicket.dart';
...
class _ChatQueryAdminState extends State<ChatQueryAdmin> {
  ...
  final List<String> tabLabels = ["Open", "Closed"];
  String selectedTab = "Open"; // Default to Wait tab
    ...
    UserData().getUserData().then((value) async {
      ...
      clientIssueSubscription = collection.snapshots().listen((ciDocs)async{
        if(ciDocs.docs.length != 0){
          setState(() {
            statusCount['Open'] = (ciDocs.docs.length == 0 ? [] : ciDocs.docs).where((e)=> e.data()['status']['status'].toLowerCase() == "open" || (e.data()['status']['status'].toLowerCase() == "open" && e.data()['peopleinvolved'].includes(profileId))).length;
            statusCount['Closed'] = (ciDocs.docs.length == 0 ? [] : ciDocs.docs).where((e)=> e.data()['status']['status'] == "Closed" || (e.data()['status']['status'].toLowerCase() == "closed" && e.data()['peopleinvolved'].includes(profileId))).length;
